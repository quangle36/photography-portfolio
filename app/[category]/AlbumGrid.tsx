...
const AlbumGrid: React.FC<AlbumGridProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      {albums?.map((album) => {
        console.log("album.path", album.pathname)
        // console.log('router.aspath', router)
        return (
          <div
            onClick={() => onAlbumClick(album.pathname)}
            key={album._id}
            className="group relative cursor-pointer overflow-hidden rounded-lg shadow-lg transition-transform duration-300 hover:-translate-y-1"
          >
